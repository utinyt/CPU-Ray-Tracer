...
void ReadScene(const std::string inName, Scene* scene)
{
    ...
    for (std::string line; getline(input, line); ) {
        ...
        for (std::string s; lineStream >> s; ) { // Parses space-separated strings until EOL
            ...
            if (!(std::stringstream(s) >> f)) f = static_cast<float>(nan("")); // An alternate that produced NANs
            floats.push_back(f);
            strings.push_back(s); }
...
int main(int argc, char** argv)
{
    ...
    for (int i = 0; i < 2; ++i) {
        ...
        // RayTrace the image
        scene->TraceImage(image, passes[i]);

        // Write the image
        WriteHdrImage("output" + std::to_string(passes[i]) + "pass.hdr", scene->width, scene->height, image);
    }
